...
// Required when running behind a reverse proxy such as Render.
...
// Use DATA_DIR for persistent hosting storage.
// Locally, it defaults to the same folder as server.js.
...
app.get("/api/articles/:id", (req, res) => {
  ...
  // The client can render the markdown.
  // Keep the source markdown here.
  res.json(row);
});
...
app.post(
  ...
  (req, res) => {
    ...
    if (parentId !== null) {
      ...
      // Replies stay flat under the original top-level comment.
      // If the target is itself a reply, use its root comment.
...
app.delete(
  ...
  (req, res) => {
    ...
    // Every reply belongs to the same flat thread as its
    // top-level comment.
    // Deleting any comment therefore removes the whole thread.
...
// Protects the login endpoint without another dependency.
// The counters reset when the server restarts.
...
app.post(
  ...
  async (req, res) => {
    ...
    // Successful login clears failed attempts for this IP.
...
app.post(
  ...
  async (req, res) => {
    ...
    if (username.toLowerCase() === "daniel") {
      return res.status(400).json({
        error: "Daniel is reserved for the administrator."
      });
    }
...
app.delete(
  ...
  (req, res) => {
    ...
    if (
      target.is_admin ||
      target.username.toLowerCase() === "daniel"
    ) {
      return res.status(400).json({
        error:
    ...
    // Find every thread containing comments
    // authored by this user.
    const threadRoots = db
      .prepare(`
        SELECT DISTINCT
    ...
    res.json({
      ok: true,
      threadsDeleted: threadRoots.length
    });
  }
);
...
app.listen(port, () => {
  console.log(
    `Daniblog is running at http://localhost:${port}`
  );
});
